...
export class King {
  ...
  static register(name: string, obj: General<any> | Soldier<(...args: any) => Promise<any>>) {
    if (obj.type === 'general') {
      if (King.generals.has(name))
        throw new Error(`General [${name}] has been registered`)
      King.generals.set(name, obj as General<any>)
    }
    if (obj.type === 'soldier') {
      if (King.soldiers.has(name))
        throw new Error(`Soldier [${name}] has been registered`)
  ...
  static find(name: string, type?: 'soldier' | 'general'): General<any> | Soldier<any> | void {
    if (!type)
      return King.generals.get(name) || King.soldiers.get(name)
